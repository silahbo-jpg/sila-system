...
type AmbulanteTipo = 'alimentacao' | 'artesanato' | 'vestuario' | 'outro';

interface AmbulanteFormData {
  nome: string;
  bi: string;
  telefone: string;
  email: string;
...
const CadastroAmbulante: React.FC = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<AmbulanteFormData>({
    nome: '',
    bi: '',
    telefone: '',
    email: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Dados do ambulante:', formData);
      
      // Simulando uma requisição assíncrona
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      alert('Cadastro de ambulante realizado com sucesso!');
      navigate('/dashboard');
    } catch (error) {
      console.error('Erro ao cadastrar ambulante:', error);
      alert('Ocorreu um erro ao cadastrar o ambulante. Tente novamente.');
    } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label htmlFor="bi" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Nº do BI <span className="text-red-500">*</span>
                </label>
                <Input
                  id="bi"
                  name="bi"
                  ...
                  value={formData.bi}
                  onChange={handleChange}
                  placeholder="Ex.: 0000000000LA000"
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="tipo" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Tipo de Comércio <span className="text-red-500">*</span>
                </label>
                <Select
                  id="tipo"
                  name="tipo"
            ...
            <div>
              <label htmlFor="descricao" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Descrição do Comércio <span className="text-red-500">*</span>
              </label>
              <textarea
                id="descricao"
                name="descricao"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Selecione no mapa abaixo a localização exata do seu comércio.
...
export default CadastroAmbulante;
